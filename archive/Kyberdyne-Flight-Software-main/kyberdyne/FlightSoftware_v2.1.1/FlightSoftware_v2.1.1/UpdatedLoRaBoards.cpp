...
void Debug::printSatBeforeCRC(const uint8_t* message, size_t len)
{
    DEBUG_BLOCK({
        Serial.println("From combineHeader:");
        Serial.print("Message before CRC: ");
...
void Debug::printCRC(const uint8_t* crc)
{
    DEBUG_BLOCK({
        Serial.println("From crc:");
        Serial.printf("CRC generated: %02X %02X\n", crc[0], crc[1]);
        Serial.println();
    })

}

void Debug::printFullMessage(uint8_t* final_msg, size_t len)
{
    DEBUG_BLOCK({
        Serial.println("From finalSend:");
        Serial.print("Sending: ");
        for (size_t i = 0; i < len; ++i) 
        {
            if (final_msg[i] < 0x10) Serial.print("0");
            Serial.print(final_msg[i], HEX);
        }
        Serial.println();
        Serial.println(); 
    })
...
void Debug::printStartCoords(float* launchCoords)
{
    DEBUG_BLOCK({
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.println("From launchCoords:");
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.print("Start lat: ");
        Serial.print(launchCoords[0], 6);
        Serial.print("    ");
        Serial.print("Start lng: ");
        Serial.println(launchCoords[1], 6);
        Serial.println();
    })

}

void Debug::printData(uint32_t enc_time, float* data)
{
    DEBUG_BLOCK({
        // adjust time
        uint8_t hrs = enc_time / 1000000;
        uint8_t min = (enc_time / 10000) %100;
        uint8_t sec = (enc_time / 100) %100;

        Serial.println("From readAndSend:");
        Serial.println("  Time       Lat       Lng       Alt      Temp      Press");
        Serial.printf("%02u:%02u:%02u", hrs, min, sec);
        Serial.print("    ");
        for(int i = 0; i < 5; i++)
        {
...
void Debug::printSatCheck(float time_bw, unsigned long interval)
{
    DEBUG_BLOCK({
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.println("Currently running satComCheck");
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.println();
        Serial.print("Interval is: ");
        Serial.println(interval);
        Serial.print("Time between messages is: ");
        Serial.println(time_bw);
        Serial.println();
})

}

void Debug::printCircGeoFence(bool within, float *startCoords, int checksLeft, float radius, float distance)
{
    DEBUG_BLOCK({
        Serial.println("From circGeoFence:");
        if(!within)
        {
            
            Serial.print("Start lat: ");
            Serial.print(startCoords[0], 6);
            Serial.print("    ");
            Serial.print("Start lng: ");
            Serial.println(startCoords[1], 6);
            Serial.println("Outside inclusion zone.");
            Serial.print("Checks left: ");
            Serial.println(checksLeft);
            Serial.print("Radius (km): ");
        ...
        } else if(within)
        {
            ...
            Serial.print("Start lng: ");
            Serial.println(startCoords[1], 6);
            Serial.println("Inside inclusion zone.");
            Serial.print("Checks left: ");
            Serial.println(checksLeft);
            Serial.print("Radius (km): ");
            Serial.println(radius,6);
...
void Debug::printZoneGeoFence(float lat, float lng, int zone, bool inZone, bool checkingLastZone)
{
    DEBUG_BLOCK({
        // Serial.println("From zoneGeoFence:");
        if(checkingLastZone)
        {
            Serial.println("------------------------------------------------------------------------");
            Serial.println();
            Serial.print("Checking Last Known Zone, which was zone");
            Serial.println(zone+1);

            if(inZone)
            {
                Serial.println("TBeam is inside the Last Known Zone");
                Serial.print("Lat: ");
                Serial.print(lat,6);
                Serial.print("    ");
                Serial.print("Long: ");
            ...
            {
                Serial.println("TBeam is outside any geofences.");
                Serial.print("Lat: ");
                Serial.print(lat,6);
                Serial.print("    ");
                Serial.print("Long: ");
        ...
        else
        {
            Serial.println("------------------------------------------------------------------------");
            Serial.println();
            Serial.print("Checking zone");
            Serial.println(zone+1);
            if(inZone)
            {
                Serial.println("TBeam is inside a geofence zone");
                Serial.print("Lat: ");
                Serial.print(lat,6);
                Serial.print("    ");
                Serial.print("Long: ");
...
void Debug::printChecksLeft(int checksLeft)
{
    DEBUG_BLOCK({
        Serial.print("Checks left: ");
        Serial.println(checksLeft);
...
void Debug::printStartMission()
{
    DEBUG_BLOCK({
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.println("Currently running startMission");
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.println();
    })
...
void Debug::printRunMission()
{
    DEBUG_BLOCK({
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.println("From runMission");
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.println();
    })
...
void Debug::printAfterTermination()
{    
    DEBUG_BLOCK({
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.println("From afterTermination");
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.println();
    })
}

void Debug::timerEnded(unsigned long timeElapsed, unsigned long timeLimit)
{
    DEBUG_BLOCK({

        float finalTime = timeElapsed / 1000.0f / 60.0f;
        float limit = timeLimit / 1000.0f / 60.0f;
        Serial.println("--------------------------------------------------------------------------------------------");
        Serial.println("From timer.check");
        Serial.println("--------------------------------------------------------------------------------------------");
